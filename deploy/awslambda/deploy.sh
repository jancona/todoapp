...
aws cloudformation package --template-file todo.cf.yaml --output-template-file output-todo.cf.yaml --s3-bucket $s3_bucket_name
aws cloudformation deploy --template-file output-todo.cf.yaml --stack-name $STACK_NAME --parameter-overrides "DomainName=${DOMAIN_NAME}" "CertificateArn=${CERTIFICATE_ARN}" --capabilities CAPABILITY_IAM
